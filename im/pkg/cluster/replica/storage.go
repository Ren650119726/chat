...
import "chat/im/pkg/logger"

type IStorage interface {

	// TruncateLogTo  截断日志, 从index开始截断,index不能等于0 （保留下来的内容不包含index） [1,2,3,4,5,6] truncate to 4 = [1,2,3]
...
func NewMemoryStorage() *MemoryStorage {
	return &MemoryStorage{
		...
		Log:               logger.NewLog("replica.MemoryStorage"),
	}
}
...
func (m *MemoryStorage) LastIndexAndTerm() (uint64, uint32, error) {
	return m.lastIndex(), 0, nil
}
